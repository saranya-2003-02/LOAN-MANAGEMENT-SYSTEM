...
alter table customer_info rename applicant_income_grade;
create table customer_info as select loan_id,`customer id`,applicantincome,coapplicantincome,property_area, loan_status,
case
...
-- Calculating monthly interest percecntage based on income
create table interest_percentage select *, 
case 
...
when applicantIncome < 5000 and property_area = 'Semi Urban' then applicantIncome*2.5/100
else applicantIncome*7/100
end as interest_percentage from customer_income;
...
--  primary table
create table loan_status (Loan_ID varchar(40) primary key,	Customer_id varchar(40), 
Loan_amount varchar(100), Loan_Amount_Term int,Cibil_Score int);
desc loan_status;
...
create table loan_status_2 (Loan_ID varchar(40) primary key,	Customer_id varchar(40), 
Loan_Amount varchar(100), Loan_Amount_Term int,Cibil_Score int);

create table cibil_score_2 (loan_id varchar(40), loan_amount varchar(100), 
cibil_score int, cibil_score_status varchar(100));

insert into loan_status_2 select * from loan_status;

insert into cibil_score_2 select * from cibil_score;
...
delete from cibil_score_2 where loan_amount= "loan still processing" or cibil_score_status = "Reject customers(Cannot apply loan)"; 

-- Update loan as integers
alter table cibil_score_2 modify loan_amount int;

-- creating all the above fields as table
create table loan_cibil_score_status_details (loan_id varchar(40),loan_amount int, 
cibil_score int, cibil_score_status varchar(100));
insert into loan_cibil_score_status_details select * from cibil_score_2;

select * from loan_cibil_score_status_details;

-- caluclation monthly interest
create table monthly_interest select i.*,l.loan_amount,l.cibil_score,l.cibil_score_status,
case 
when applicantIncome<5000 and Property_Area = "rural" then l.loan_amount *3/100
when applicantIncome<5000 and Property_Area = "urban" then l.loan_amount * 5/100
when applicantIncome<5000 and Property_Area = "semi urban" then l.loan_amount*2.5/100
else l.loan_amount*7/100
end as monthly_interest_calc
from loan_cibil_score_status_details l inner join interest_percentage i  on l.loan_id=i.loan_id;

select * from interest_percentage;
select * from monthly_interest;

-- annual intererst calculation
create table customer_interest_analysis select *,monthly_interest_calc*12 
...
create table output_1 select c.*, l.loan_amount,loan_amount_term,cibil_score,d.customer_name,d.gender,
d.age,d.married,d.education,d.self_employed,d.region_id,s.postal_code,s.segment,s.state,
r.region from customer_income c 
left join loan_status l on c.customer_id = l.customer_id 
left join customer_det d on d.customer_id = c.customer_id
left join country_state s on s.customer_name = d.customer_name
